...
Lendkey.repo_paths.each do |path|
  puts path
  Dir.chdir(path) do
    `git log --shortstat --no-merges --since=07/16/2013 --pretty=format:"%an" -- {spec,features}`.split("\n\n").each do |line|
...
File.open('/Users/kstannard/spec_commit_counts.yml', 'w') {|f| YAML.dump(counts, f)}
